...
for (let i = 5; i >= 1; i--) {
  const count = ratingCounts[i as keyof typeof ratingCounts];
...
data.reviews.forEach(review => {
  if (review.details) {
    categories.forEach(cat => {
      if (review.details && review.details[cat as keyof typeof review.details]) {
        if (!categoryStats[cat]) {
          categoryStats[cat] = { sum: 0, count: 0 };
        }
        categoryStats[cat].sum += review.details[cat as keyof typeof review.details] as number;
...
const timePeriods: { [key: string]: number } = {};
...
sortedPeriods.forEach(([period, count]) => {
  const bar = "█".repeat(Math.round((count / Math.max(...sortedPeriods.map(p => p[1]))) * 40));
